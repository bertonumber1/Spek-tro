...
void spek_pipeline_finish(struct spek_pipeline *p)
{
    // The reader loop ends on its own at end of file; joining without raising
    // `quit` is therefore a wait rather than an abort.
    if (p->has_reader_thread) {
